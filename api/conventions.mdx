...
Keep identifiers within the environment and tenant that returned them. A hostname selects the environment; the `customer` header selects the tenant.
...
| `401` | Check the API key and selected environment. |
| `403` | Check tenant selection and the key owner's access to the requested resource or action. |
| `404` | Verify the resource ID in the selected project, tenant, and environment. |
