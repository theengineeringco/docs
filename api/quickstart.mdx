...
The response contains an `items` array and pagination metadata. Omit `branch_id` to read Base. See [Read entities](/api/examples/read-entities) to select fields, filter results, and fetch the next page.

## Try the playground

Open an endpoint under **API reference**, select your environment, and enter your API key and `customer` header. Start with a GET request. The playground sends requests to the selected environment using your permissions; write requests change that environment's data.
